...
func load_task(a_ItemID : int) -> void:
	var Icon_path = "res://System_task_manager/Assets/" + DataHandler.task_data[str(a_ItemID)]["Name"] + ".png"
	IconRect_path.texture = load(Icon_path)
	for grid in DataHandler.task_grid_data[str(a_ItemID)]:
		var converter_array := []
		for i in grid :
